...
class TGService(
    ...
    @Transactional
    suspend fun sendMessageToChannel(channelId: String, text: String, parseMode: String? = null): Message {
        logger.info("Sending message to channel $channelId")
        val res = bot.sendMessage(channelId, text, parseMode=parseMode)
        return res
    }

    @Transactional
    suspend fun sendMessageToSubscriber(source: SourceType, text: String, parseMode: String? = null) {
        ...
        channels.forEach {
            ...
            try {
                it.id?.let { it1 -> this.sendMessageToChannel(it1, text, parseMode) }
